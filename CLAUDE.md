...
- Contains plugin entries: single-skill plugins point `source` directly at the skill directory (no `skills` field); suite plugins (`daymade-audio`, `daymade-claude-code`, `daymade-codex`, `daymade-docs`, `daymade-financial`, `daymade-macos`, `daymade-skill`) use explicit `skills` arrays for multi-skill routing
...
- **All suites are suite-only.** `daymade-audio`, `daymade-claude-code`, `daymade-codex`, `daymade-docs`, `daymade-financial`, `daymade-macos`, and `daymade-skill` do NOT register their member skills as standalone plugins. Users install the suite (e.g., `daymade-audio@daymade-skills`) and invoke skills as `<suite>:<skill>` (e.g., `daymade-audio:transcript-fixer`, `daymade-macos:macos-cleaner`). When adding a new skill that belongs to a suite, only update the suite entry's `skills` array — do NOT create a parallel standalone plugin entry.
