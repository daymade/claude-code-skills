...
仅当上层应用明确给出研究问题、上轮完整报告、未决状态、有效纠正、信息截点和结构化输出合同，
并声明本轮为只读宿主时走此分支。继续执行本 Skill 的 Tibo 信源发现、上下文补证、
`global` / `banked` / `both` / 未明分类及不同用量决策；公开来源清单仍只是起点。
应用的持久研究状态代替本机 `forecast_log.py` 台账：本轮不调用 `finding` / `record` /
`review` 写入，也不读取用户目录下可能属于另一研究范围的默认台账。把未决承诺、候选来源、
类型、窗口与下一次可执行复查条件写入宿主要求的 `state.entries`，在完整报告中保留原帖和
覆盖缺口；下一轮由应用把它们原样带回。只有宿主**明确列出**已获授权的私人来源时才按
`read-wechat-messages` 的账号预检和媒体读取合同访问；没列出、预检失败或只读环境无法完成
图片/语音处理时标 `uncovered`，不能用公开来源的空值代答。宿主产物里的变化摘要是待核研究，
不冒充已送达、已读或正式账户操作。独立调用本 Skill 仍按上文写本地 findings 和预测台账。
