...
### 44. **mindbreak** - 工作健康休息提醒

追踪你使用 Claude Code 的连续工作时长，在回复末尾自然地插入休息提醒。通过轻量 hook 在每条消息时记录时间戳，计算连续工作时长，达到阈值后触发提醒——无需任何外部依赖。

**适用场景：**
- 长时间编码容易忘记休息
- 经常错过饭点或不知不觉工作到深夜
- 希望有不打扰工作流的、上下文感知的健康提醒

**核心功能：**
- 基于 hook 的活动追踪，自动识别工作段（离开 15 分钟 = 新的工作段）
- 三种提醒类型：轻度休息（45+ 分钟）、饭点提醒（11:30/17:30）、深夜���醒（21:00+��
- 智能冷却：两次提醒间隔至少 30 分钟，每个工作段最多 3 次
- 自包含的 `<IMPORTANT>` 标签输出，在长上下文中也能可靠触发
- 提醒出现在 Claude 回复的最后一段——不会打断正常内容
- 说"不用提醒"或"stop reminding"即可关闭当次会话的提醒
...
📚 **文档**：参��� [mindbreak/SKILL.md](./mindbreak/SKILL.md)。
