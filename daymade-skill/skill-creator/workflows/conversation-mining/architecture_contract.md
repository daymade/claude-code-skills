...
Do not instantiate every role merely because it exists. A small, bounded corpus is handled inline from the redacted chunks by default; if an agent is justified, give it one narrow role. Multiple agents require distinct outputs plus the main SKILL.md agent-budget gate. Agents do not call tools; they receive a redacted chunk and produce structured markdown.
