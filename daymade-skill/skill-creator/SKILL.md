...
- **Corollary — your fixes are themselves a defect source, so re-check the regions you touched, after the last edit.** In a two-round review of a single change, round 1 produced eight findings; round 2 produced eight more, **seven of which were cross-reference errors introduced by round 1's fixes**. Rewriting a sentence moves the anchors around it. This is also why "it was reviewed" is not a finish line for an artifact whose readers navigate by its pointers: the review is stale the moment you act on it. The re-check is mechanical. **YOURDIFF** — the one form to use whenever this block says "your own hunk", because every shorter spelling has a measured blind spot:

```bash
BASE=$(git rev-parse <the commit you started from>)   # NOT HEAD — see below
git ls-files --error-unmatch <file> \
  && git diff -U0 "$BASE" -- <file> \
     | grep -E '^\+' | grep -v '^+++' \
     | grep -Ei '(see|per|above|below|§|rule|discipline|step) *#?[0-9]|\]\([^)]+\)'
```

Resolve each hit against its target. Every piece is there because its absence was measured to produce a *silent* pass — a check that reports success having examined nothing:

- **`$BASE`, not `HEAD`.** Bare `git diff` sees only unstaged work, so one `git add` hides your edit. `git diff HEAD` fixes that but breaks the moment you **commit** — and this bullet tells you to re-check *after the last edit*, which most people read as after committing. Measured: a file containing a real, unverified `see discipline #6`, already committed, yields **0 bytes and exit 1** under `git diff HEAD` — byte-identical to a clean tree. Anchor to where your work started and all three states (unstaged, staged, committed) stay visible.
- **`ls-files --error-unmatch` first.** A mistyped or wrong-relative path makes `git diff` print nothing and exit 0, indistinguishable from success; this turns it into exit 1 plus `pathspec … did not match any file(s) known to git`. ⚠️ That message goes to **stderr** — an agent capturing only stdout sees an empty string either way, so read the exit code or keep stderr attached.
- **`grep -v '^+++'`.** The diff's own `+++ b/<path>` header is a `^\+` line; when the path contains something like `step2/`, the net matches the *filename* and reports a hit in a file whose added content has none.
- **No `-n` on the last `grep`.** After the first filter it numbers the *piped stream*, not the file — authoritative-looking numbers pointing nowhere.

And read empty output as a question. Even with all of the above it still means **one of three** things — nothing changed; changed but nothing looked like a pointer; or your `$BASE` is wrong and you are diffing against your own work. The exit code cannot separate them (`ls-files` failure and grep's no-match both exit 1), so decide which one you are in before calling it a pass. It costs seconds and does not depend on vigilance, which is what discipline #6 asks of any check worth writing. (It is a net, not a proof — it will miss a pointer phrased as prose, so read the added lines too.)
