...
### Case 5: 否定判断也要留痕——别让分类 skill 重判（2026-06）
...
### Case 6: 规则齐全也会犯——示例里混入真名（2026-06-26）

优化一个 PUBLIC repo 的 skill 时，把刚处理过的真实项目人名（CJK）顺手写进了示例 `--add` 命令和词典规则举例——客户隐私直接进开源。讽刺的是该 repo 的隐私规则**早就写明**「示例里的真实 CJK 名也要脱敏、gitleaks 不覆盖 CJK、靠 AI 语义通读」——规则齐全，但写示例时没把「例子」当敏感内容过闸。
