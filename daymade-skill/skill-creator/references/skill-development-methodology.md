...
决定性的一条是：**收信方可能根本没装这个 skill。** 它手上只有宿主那句「抄 `from`」和官方工具，而官方列表里的短标识不是 session-id 的前缀，对不上。**接收侧不存在恢复路径**——文档到不了真正受害的那一方。
...
- **not-found 分两种。** 「与系统状态无关的畸形参数」和「实体不存在」是不同的失败（gRPC 的 `INVALID_ARGUMENT` 与 `NOT_FOUND`；DNS 分得更细，NXDOMAIN / NODATA / referral 三种）。用错空间拿到的 not-found 不是存在性结论。但这条**不许退化成「永远别信 not-found」**：查询发生在对的空间、且该码在协议上被定义为权威否定时，它就是终点，继续换地址试就是无限搜索。
- **不要把接收器改宽容。** 「让官方工具也吃我这套形式」就是 Postel 的 be-liberal，而宽容会掩盖问题、让错误固化（RFC 9413 的 virtuous intolerance）。正解是显式换算加精确报错。

**触发条件（防误伤）**：只在**同一个能力存在两条及以上 route** 时适用。单路由场景没有第二套空间，不需要任何换算，也不需要怀疑 not-found。
