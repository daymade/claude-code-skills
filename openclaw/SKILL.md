...
description: >-
  ...
  file, or when they mention lobsters, 虾, 剪辑虾, 标题虾, DeepSeek patch,
...
```json
{
  "剪辑虾": "/path/to/剪辑虾/openclaw.json",
  "标题虾": "/path/to/标题虾/openclaw.json"
}
```
...
```bash
python3 scripts/cli.py copy gateway-provider --from 剪辑虾 --to 标题虾 --alias
python3 scripts/cli.py switch gateway-provider/deepseek-v4-pro --config 标题虾 --restart
```
...
```bash
python3 scripts/cli.py audit --config 标题虾
python3 scripts/cli.py add-model gateway-provider deepseek-v4-pro \
  --config 标题虾 --from 剪辑虾 --alias "DeepSeek V4 Pro"
python3 scripts/cli.py switch gateway-provider/deepseek-v4-pro --config 标题虾 --restart
```
...
```bash
python3 scripts/cli.py copy gateway-provider --from 剪辑虾 --to 标题虾 --alias --restart
python3 scripts/cli.py audit --config 标题虾
```
...
```bash
python3 scripts/cli.py diff 剪辑虾 标题虾
```
