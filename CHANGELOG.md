...
### Fixed
- **prior-work-retrieval** (`daymade-claude-code` v1.57.1): make its registered Linux test suite independent of a runner-installed ripgrep binary by exercising the existing `rg --json` adapter contract through a test-only executable fixture. Production behavior is unchanged: a real missing `rg` still fails coverage explicitly, now protected by a dedicated regression. The exact GitHub failure shape moved from 2 failures plus 7 errors to 25/25 passing tests in a clean Python 3.12 Linux container.
