...
### Changed
- **skill-creator** (`daymade-skill` v1.27.2): align every security layer with the shared packaging policy. Scans stage the complete package superset (including optional `evals/`) while excluding non-shipping `.enrich/`, `tests/`, and `dist/` artifacts, so retained conversation-mining evidence cannot block a release and packaged content cannot escape attestation. Gitleaks findings are remapped to actionable source paths; nonzero scanner exits without findings fail closed; the marker binds the exact staged bytes gitleaks examined; and regex checks cover every shipping UTF-8 text file, including hidden, extensionless, and HTML files, while skipping binary content. Regression tests pin the staging boundary, source-path remapping, failure exits, mutation race, hidden/text-file coverage, and binary skip; the full skill-creator suite passes.
