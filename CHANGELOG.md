...
### Changed
- **transcript-fixer** v1.33.1 → v1.33.2: two exits that recorded something other than what happened. `harvest_corrections.py` aborted the whole harvest (exit 2, nothing printed) when one candidate could not be rendered as a trap bullet — a vendor's `***` redaction mask diffed against the real word yields a FROM the bullet grammar cannot carry, since `*` is the bold delimiter and `_BOLD_TRAP` rejects it on either side even inside backticks. On a real two-hour transcript that single pair hid every other candidate; the same pair now prints 45. The noise filter drops any side with no letter or digit in any script (`str.isalnum`, so fullwidth `ＡＩ`, kana and hangul still harvest — the ASCII/URO-only `_WORD_RUN` was the wrong instrument for this test) or containing `*`, and a bullet that still fails the parser round-trip is reported on stderr, excluded from the output and from `--write`, and no longer aborts the run (`--json` lists it under `unrenderable`). The review queue's accept path fails closed with `ReAnchorNeeded` once the anchor text is gone; a fix applied by hand before the verdict was indistinguishable from file drift, and SKILL.md told the agent to close such rows with `kept_original` — a verdict describing a transcript that was right as spoken, contradicting `native_ai_full_workflow.md` 9b, which reserves `kept_original` for the actual human verdict. `--decision accepted` (and `overridden`, for its override text) now recognises the one shape it can tell apart: the original is gone from the whole ledger-masked file and the context recorded at enqueue reappears anywhere in it with the resolved text in the exact slot the original occupied, recorded neighbours intact on both sides (width tried from 8 characters down to 2; a one-sided neighbourhood needs 3). The whole file is searched rather than the ±3-line resolve window: frontmatter growth shifts every later line, and with the original gone `--reanchor-review` has nothing to re-locate — a fresh-context review of the first cut hit exactly that dead end. The same review caught the window check accepting a row whose anchored line had been hand-edited to a third form while a sibling line already carried the corrected phrase; at the width that matches, every occurrence of the recorded neighbourhood in the file must now carry the resolved text, or the check fails closed. It records the verdict without writing and logs `already in place at the anchor — recorded without writing`; any other shape still raises `ReAnchorNeeded`, the `asr_note` ledger mask still applies, and `reopen` reverts nothing it never wrote. The three runtime messages that still named `kept_original`/`skipped` as the exit for a missing original, the `ReAnchorNeeded` docstring and the dashboard reference's fail-closed paragraph now describe the recognised shape and its limits. SKILL.md item 7, the native workflow reference and `script_parameters.md` say to close Stage 1 deferrals through their queue rows (ids from `--list-review --review-file`) rather than with sed, that a row the queue can neither re-anchor nor recognise closes with `skipped` plus a note, and that `kept_original` asserts the transcript keeps the original form. Seventeen deterministic tests cover both changes, including the sibling-third-form and past-the-window cases and a pinned test for the one shape the check cannot see (anchored utterance deleted while a twin already reads corrected — recorded as accepted with nothing written); three pre-existing `test_stage1_auto_finalize` failures and three collection errors under `scripts/tests` are unchanged on the baseline and out of scope.
