...
### Changed
- **transcript-fixer** v1.33.4 → v1.34.0: the sidecar lifecycle gets a closing command and Stage 1 stops matching fragments across real words. (1) `--close-sidecars --input FILE` decides `closed | open | blocked` mechanically: every `*_changes.md`/`*_needs_review.md` entry is re-read against the ledger-masked transcript (applied / still the original / anchor rewritten past both forms) and against the review queue for that exact file (a decided row closes the pair, a pending row blocks), a `*_stage1.md` newer than the file is refused (the plain Stage 1 rerun is its promotion path), a newer `*_stage2.md`/`*_dryrun.md` is retained unless `--discard-unpromoted`; only `closed` removes the evidence, and `--decide-raw kept_original|skipped --by --note` records verdicts through the queue so closure leaves an audit trail rather than a silent deletion (exit 0/1/2, `--dry-run`, `--json`). Until now the contract said "preserve until every associated decision is closed" and nothing could decide it: a 51-sidecar audit of one knowledge base on 2026-09-05 found 36 files from 20 transcripts whose decisions had long closed, plus API `_stage2.md` outputs that were never promoted and never discarded. (2) Safety layer 2b in `DictionaryProcessor`: a dictionary match that begins or ends inside a multi-character segment of a dictionary-only jieba cut (HMM off) is refused and counted as `Refused at word boundaries`, neither applied nor deferred — 新一 in 更新|一下, 同龄 in 同龄人, `Cloud` in `iCloud`. Replay on three real transcripts against the pre-change engine: 8 refusals, all fragments, no correction lost; five of them were `Cloud→Claude` inside `iCloud` that the old engine auto-applied as low risk, so the recommended plain Stage 1 rerun would have written `iClaude` five times. jieba was already a declared dependency (the 4+ char real-word guard). (3) `--lookup TERM`: every existing claim on a term — dictionary rules active and disabled as FROM or TO, context rules, roster variants, review-queue rows — replacing a corpus probe or raw SQL for "is there already a rule for X". (4) Docs: `_stage2.md`/`_dryrun.md` are run-scoped outputs, promoted or discarded in the session that produced them; SKILL.md step 10 and Finalization, troubleshooting §7, the native workflow's finalize step and rung 1, script_parameters and false_positive_guide carry the new contract. 30 new tests (731 total).
