...
### Changed
- **peer-message** v1.0.1 → v1.1.0: add an operational coordination and learning contract distilled from post-release Claude/Codex use. Parent tasks now propagate an exact reply address; multiline or data-rich reports use the CLI's UTF-8 message-file entry; status language stops at transport, receiver queue/history, explicit reply, or independently verified task completion instead of collapsing them into “delivered.” The new evidence-gated improvement loop admits receiver records, task outcomes, deterministic tests, and user corrections, routes each failure to its owning layer, requires executable When/Do/Evidence/Missing/Do-not-infer/Stop rules, and defines the boundary between bounded Skill evolution and unsupported autonomous RSI. Existing transports, receipts, exit codes, authorization boundaries, and CLI behavior are unchanged.
