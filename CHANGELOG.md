...
### Added
- **transcript-fixer** v1.34.1 → v1.34.2: two roster-variant shapes can no longer become global rules, and a retired rule no longer keeps a sidecar open. (1) The people-roster loader refuses a **single surname plus an honorific** (`朱老师`, `王总`) with a loud stderr warning, and `--add` / `--import` refuse the same FROM shape as an error in every mode: such a form names everyone with that surname, so one meeting's mishearing of one person's surname recorded as a variant rewrites people who were named correctly. Real case 2026-09-07: seven surname variants captured from a single meeting had been loaded as global rules for months, and a `朱老师` in an unrelated transcript was presented as that person; the rule had already been disabled as a false positive, but the roster line resurrected the question. A given name plus an honorific (`明源总`) and a bare misheard name token (`小铭`) still load. (2) The **bare-numeric refusal** written on 2026-09-05 ships with it: a variant that is only digits (`95`) is refused at roster load and by `--add` / `--import`, because digits match timestamps, scores and prices in every transcript (one such line once deferred 122 items in a single rerun). The three predicates — loader, `check_correction_safety`, import — are kept in sync and documented together in `dictionary_identity_and_context.md`; the native workflow gains the entity-evidence rule that parallel list slots are not identity evidence. (3) `--close-sidecars` reads the dictionary's disabled pairs: a report entry whose FROM→TO rule has since been disabled as a false positive is `disabled` — closed without a verdict and never re-asked — instead of surfacing as `undecided` for a human to answer again; `entries.disabled` is added to the JSON. 6 new tests (751 collected across `scripts/tests` and `tests`); the roster fixture's example variant is renamed because the old example (`金老师`) is itself a refused shape.
