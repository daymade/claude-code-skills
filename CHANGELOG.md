...
### Changed
- **skill-creator** (`daymade-skill` v1.27.2): align gitleaks with the shared packaging policy. Security scans now stage the complete package superset (including optional `evals/`) while excluding non-shipping `.enrich/`, `tests/`, and `dist/` artifacts, so retained conversation-mining evidence cannot block a release and packaged content cannot escape attestation. Gitleaks findings are remapped from the temporary staging tree to actionable source paths; the pattern scanner and content hash now consume the same file iterator. Three regression tests pin the staging boundary, source-path remapping, and hidden shipping-file coverage; the full skill-creator suite passes.
