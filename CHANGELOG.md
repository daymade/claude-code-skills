...
### Fixed
- **prior-work-retrieval** (`daymade-claude-code` v3.2.0): second hardening pass, driven by a four-agent audit of the gate's own recorded state rather than by imagination, plus a `scripts/prior_work.py audit` command so the next such diagnosis is one command instead of an archaeology session (it reports trigger mix, empty-gate rate, stranded receipts, non-user arms, and the matched token behind every still-arming entry; `--json` for machines). Three classes of real defect were confirmed against disk and fixed. **Gates nobody could satisfy:** recorded sub-agent prompts that forbid reading skills or running the shell were armed anyway — two said outright `Do NOT perform prior-work retrieval` and `The user explicitly opts out of prior-work retrieval` and were still gated, because the opt-out pattern accepted only `skip`/`disable` and only the space-separated spelling. A prompt stripped of the capability to complete a receipt is now never gated, and opt-out is honoured in the spellings people actually use. Two further non-user channels: a background workflow's own `<task-notification>` reaches UserPromptSubmit (found live while this work ran), and another hook's block message echoed back as a prompt (`• UserPromptSubmit (blocked) says: …`) — this hook's own guidance was already excluded, every other hook's was not. Harness envelopes (`-message`, `-notification`, `-reminder`) and that echo shape join the filter, while a user prompt merely opening with a tag or a bullet (`<div> 这个组件我们之前写过吗？`) still arms. One audited finding was overturned rather than shipped: a standing SOP template was reported as a live gap based on a reference file's wording, but all 39 of its real recorded submissions are a shorter variant that does not contain the matching token and classify as none — no change made. **Missed reuse asks** — the literal term table failed on ordinary phrasing: `不希望你重新造轮子` (only `不要重新`/`不要重造`/`别重复` were listed), `既有资产` (only `已有`/`现有`), and `成功的经验` (the 的 particle broke `成功经验`). All three now arm. **Over-firing introduced by the previous pass:** the hedge tier fired on `这个脚本好像是死循环`, an ordinary bug report — it now requires a distal or indefinite determiner (那个/某个/哪个), since 这个 points at the object in front of you while 那个 points at a remembered one. Dating something as old (`这些 Skill 也是很久之前写的`) is excised like a negation. A term-by-term audit proposing 44 further narrowings of the strong tier was rejected: strong-signal false positives cost one redundant retrieval, while narrowing costs missed reuse, and three such misses were confirmed in real state. Sweeping the stale gates those defects left behind took the empty-gate rate from 25.7% to 10.3%. 61 hook regressions (four new ones verified to fail against the previous implementation) plus 8 new audit-command tests pass; two scope-restricted sub-agent sessions verified unblocked through the installed wrapper.
- **prior-work-retrieval** (`daymade-claude-code` v3.1.0): cut four classes of false gating that made the hook fire on things the user never asked for. Measured against real state, 8 of 17 current-rule triggers were not user prompts at all, and none of the 8 ever produced a receipt: Claude Code's own safety classifier (×2) and suggestion generator (`# Overview\nGenerate 0 to 3…`, ×3), plus sub-agent role prompts (`You are a fresh-context …Reviewer`, ×3). A gated sub-agent cannot complete a receipt, so its gate only blocked its own work (`node --version` denied as an unknown executor). Inter-agent messages and pasted `⏺ Bash(…)` transcript lines are further non-user inputs reaching the same handler — one armed under the retired `required_production` rule, one did complete a receipt — and are now filtered alongside the three template families. Those now classify as non-user input and arm nothing. Negated reuse is no longer read as a retrieval request: `不要复用现在的 aicms-docs` matched on the bare token 复用, minted a fresh requirement, stranded the receipt completed 20 minutes earlier in the same session, and denied the next write as belonging to an older prompt — the negated phrase is now excised before matching, so a genuine ask in the same sentence still counts while 别重复造轮子 / 不要重新造, which ask *for* reuse, keep arming. Hedge recall (上次 / 好像是 / 我记得是 / 记不清) now requires a concrete work noun, and bare `history` requires a carrier, so `好像是下载没有进度条`, `git history`, `browser history`, and this repo's own `read-claude-code-history` skill name stop triggering while `上次做的方案叫什么来着` and `conversation history` still do. A session holding a valid receipt no longer re-arms on hedge-phrased follow-ups, though an explicit new prior-work ask still does. Four regressions cover the production incident end-to-end; 49 focused tests, the hook selftest, and a six-case end-to-end probe through the installed wrapper pass — a still-gated session's write is still denied.
