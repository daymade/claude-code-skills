...
- **skill-governance** (`daymade-skill` v1.30.0): add a read-only project dual-root audit for `.claude/skills` and `.agents/skills`. It pairs direct child bundles by frontmatter `name`, hashes complete bundle content plus executable bits, reports identical copies without pretending they cannot drift later, and fails on divergent full copies. A thin compatibility router passes only with an explicit marker, exactly one backtick-quoted paired canonical `SKILL.md` target (including project directory names with spaces or non-ASCII text), a complete-read instruction, fail-visible behavior, and no second business-rules file; shared targets pass only when the whole bundle really resolves to the same canonical content, not when one side reuses the same `SKILL.md` but hides extra material next to it. Invalid identity/router state exits 2, real drift exits 1, and both a missing-root project and declared-but-empty roots fail instead of silently returning green. Nineteen standard-library regressions run in the shared CI registry and on Linux; a real project replay distinguished canonical router, identical copy, drift, symlink-hidden drift, and single-root states.
