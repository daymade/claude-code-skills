...
### Changed
- **git-safety-net** v1.8.0: close two linked-worktree retirement gaps observed in a real no-loss cleanup. Checkout discovery now measures fetch freshness from Git's common directory, so a linked worktree no longer reports a stale private `FETCH_HEAD` after the shared repository was freshly fetched; an isolated regression reproduces the old false “never fetched” verdict and passes with the fix. The retirement gate now separates Git-clean state from ignored physical files, requires uncertain ignored content to be copied because bundles cannot reach it, and verifies path removal, worktree deregistration, and recorded-HEAD recoverability after non-forced removal. Branch deletion remains a separate authorization.
