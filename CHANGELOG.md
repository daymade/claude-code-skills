...
### Fixed
- **asr-transcribe-to-text** (`daymade-audio` v1.28.1): closes four release-seal integrity gaps without changing the long-audio workflow: each leg freezes source and producer identity before execution and rejects a changed contract before publication; completed checkpoint `i` must name exactly `chunk-{i:04d}.txt`; legacy post-run sidecars are invalidated; and a repeated canonical speaker mapping is byte-idempotent while a conflicting rename fails before any artifact changes. Twenty-five deterministic long-audio regressions pass on macOS and Linux.
