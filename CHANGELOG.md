...
### Changed
- **continue-codex-work** (`daymade-claude-code` v1.50.0): close three silent briefing gaps against current Codex rollouts and give every truncation an exit, with version claims now measured on ~2,600 real rollouts (0.142.2–0.149.0) instead of a handful of samples. (1) **Turn stream drift**: the `event_msg/user_message` / `agent_message` mirror stream — the parser's only turn source — is the norm through 0.146.x and in the 0.147/0.148 alphas, but stable 0.147.0 drops it for most sessions (30/1050 residual files), so `Last User Requests` silently vanished; the parser now also reads `response_item/message` (assistant `output_text` decoded locally — the shared `extract_text` is deliberately untouched) and lets the **richer stream win** (ties go to the event stream), because the streams measurably diverge in both directions (0.142.3/0.143.0/0.144.0 keep per-step commentary only in the event stream; other files of the same versions carry more in message records). Inter-agent `response_item/agent_message` traffic is never parsed as main-thread text, and `task_complete.last_agent_message` remains a tail safeguard. (2) **Files Edited vanished on 0.147+** the same way: `patch_apply_end` is gone (0 vs 1086 `item_completed`/`FileChange` items in one measured session), so file edits and failed-patch errors are now read from the `FileChange` envelope; both sources union into one set. (3) **Truncation had no exit**: long sections ended in a silent "..." and the Limitations section said "read the rollout line directly" without saying how — which cost a real resume session two wrong schema guesses; truncated sections now print a `rerun with --full` hint at the cut point and `--full` prints complete text. Also: skill-invocation bundles delivered as user messages (measured 2.7–148 KB) collapse to a one-line marker instead of evicting a real request; `turn_aborted` classifies as interrupted and a commentary-phase tail as in progress rather than completed. 14 synthetic-fixture tests pin the stream-selection rule in both directions, inter-agent exclusion, the `task_complete` safeguard, FileChange success/failure, the marker, the truncation contract, and the three end-reason shapes; real-corpus verified on a 187 MB / 22-compaction 0.147.0 rollout (Files Edited restored: 44 files) and dual-stream July rollouts.
