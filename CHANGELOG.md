...
### Changed
- **prior-work-retrieval / continue-codex-work** (`daymade-claude-code` v1.56.0 hotfix): close all six independently reproduced post-merge acceptance findings without adding new scope. Bash and Codex exec write paths now enter the receipt gate; an exact manifest repair remains possible while configuration is broken; installer ownership uses exact/canonical hook identity instead of substring deletion; malformed command-adapter rows are failed coverage; candidate Git HEAD drift invalidates reuse; and the selected Codex rollout identity is checked before lineage traversal.
