...
### Fixed
- **asr-transcribe-to-text** (`daymade-audio` v1.28.0): fixes the long-audio resource and orphan-process contract instead of extending the old timeout. The pinned Qwen3-ASR implementation applies `max_tokens` to each low-energy ~20-minute chunk, so the former 200,000-token default allowed one bad chunk to grow a 33.4 GB physical footprint (44.0 GB peak) and run for more than nine hours without a final file. Local MLX now defaults to 8,192 tokens per chunk, requires an explicit override above the 16,384 safety ceiling, atomically checkpoints and hash-verifies each chunk, resumes only incomplete chunks, and refuses a final transcript when any chunk exhausts its ceiling. Managed Qwen workers bind to their owner PID; `speaker_transcribe.py` gives every uv child its own process group, streams progress/heartbeats, and reaps the full tree on timeout or termination. Six deterministic tests cover the cap, checkpoint resume/corruption, partial-failure recovery, owner disappearance, and grandchild cleanup on macOS and Linux; the suite is now registered in shared CI.
