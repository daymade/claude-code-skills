...
### Fixed
- **asr-transcribe-to-text** (`daymade-audio` v1.28.0): fixes the long-audio resource, quality, provenance, and orphan-process contracts instead of extending the old timeout. The pinned Qwen3-ASR implementation applies `max_tokens` to each low-energy ~20-minute chunk, so the former 200,000-token default allowed one bad chunk to grow a 33.4 GB physical footprint (44.0 GB peak) and run for more than nine hours without a final file. Local MLX now defaults to 8,192 tokens per chunk, requires an explicit override above 16,384, atomically checkpoints each chunk, and binds resume identity to source bytes, immutable model revision, producer, splitter, sample rate, dependencies, parameters, and quality policy. A real-corpus-calibrated repetition gate rejects bounded garbage below the token ceiling. Qwen/Whisper/diarization outer caches require source+producer+parameter+artifact provenance; alignment rechecks those receipts before consumption, and an atomic final receipt binds the four delivery artifacts plus producer/model/parameter contract. Checkpoint and quality failures propagate machine-readable exit classes instead of human-message parsing. Managed workers bind to their owner PID, run in process groups, emit heartbeats, and reap descendants even when the group leader exits first. Fifteen deterministic tests pass on macOS and Linux. A 30-minute accepted production recording reran in 68.2 seconds at 4.36 GB peak RSS with zero swap and produced a byte-identical 9,005-character transcript.
