...
### Changed
- **skill-creator** (`daymade-skill` v1.27.2): align every security layer with the shared packaging policy. Scans stage the complete package superset (including optional `evals/`) while excluding non-shipping `.enrich/`, `tests/`, and `dist/` artifacts, so retained conversation-mining evidence cannot block a release and packaged content cannot escape attestation. Gitleaks findings and verbose pattern findings map to actionable source paths; contradictory or incomplete reports fail closed regardless of exit code; the internal marker writer accepts only a clean result bound to the exact staged bytes; regex checks cover every shipping UTF-8 text file while skipping binary content; and packaging revalidates that marker against an isolated snapshot and zips that snapshot, never the later live tree. Regression tests pin report/exit contradictions, both mutation windows, staging boundaries, path reporting, hidden/extensionless/HTML coverage, binary skip, and package contents; the full skill-creator suite passes.
