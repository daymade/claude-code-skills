...
### Fixed
- **skill-governance** (`daymade-skill` v1.30.1): close the remaining project dual-root audit false-clean and false-positive paths found by executable counterexamples. Declared-but-empty roots now fail instead of returning clean; a shared `SKILL.md` cannot hide extra divergent bundle material or be a router with no canonical source; router paths are parsed from one backtick-quoted project-relative target without assuming the directory matches the frontmatter name or uses ASCII; and quoting the router marker in ordinary full-Skill prose no longer misclassifies the bundle because the marker must be the first nonblank body line. Positive controls keep both directory-level and file-level symlink sharing valid. All 21 standard-library regressions pass on macOS and Linux.
