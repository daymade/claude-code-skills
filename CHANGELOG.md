...
### Changed
- **daymade-audio** v1.32.5 → v1.33.0 (`asr-transcribe-to-text`): replaces the stale “Minute URL always ends the job” rule with three explicit outcomes. Upload-only still stops at the URL; transcript-only waits for a readable Feishu transcript; project delivery hands preprocessing into `meeting-ingest` and cannot finish until routing, full correction, project indexes, verified Git handoff, and the pushed delivery receipt are complete. A blocked run preserves the same `minute_token`/URL and resumes instead of re-uploading.
