...
### Changed
- **skill-creator** (`daymade-skill` v1.26.2): third repair round on the Reference-and-self-application check, after a review of the *previous* repair (1.26.1) found that it had fixed one blind spot and left the adjacent one. (1) **`git diff HEAD` goes blind the moment you commit** — and the corollary tells you to re-check "after the last edit", which most readers take to mean after committing. Measured: a file containing a real, unverified `see discipline #6`, already committed, returns **0 bytes and exit 1**, byte-identical to a clean tree. The command now anchors to `$BASE` — the commit you started from — so unstaged, staged and committed states all stay visible. (2) **The command is now defined once**, as `YOURDIFF`, and referenced from the bullet that also needs it; 1.26.1 had guarded the corollary's copy and left the other bullet's copy bare two bullets away, in the same commit — the structural cause of that divergence is removed rather than re-patched. (3) **`grep -v '^+++'`** added: the diff's own `+++ b/<path>` header is a `^\+` line, so a path containing e.g. `step2/` made the net report a hit in a file whose added content had none (reproduced). (4) The stderr caveat is stated — `ls-files --error-unmatch` writes its loud failure to **stderr**, so an agent capturing only stdout sees the same empty string either way. (5) The "empty output means one of two things" claim is corrected to **three**, the third being "your `$BASE` is wrong", and the text now says plainly that the exit code cannot separate them. (6) The non-reproducible `120 bytes` figure is gone — byte counts depend on path length and diff content; only the direction (0 → nonzero) is portable. (7) The "name who executes this rule" bullet gains the case it was missing: when **no** tool can decide a rule — true of most prose rules — the artifact is the sentence saying so, and the bullet now states its own honest limit (it moves the judgment call rather than removing it). Each defect above was found by a fresh-context review of the preceding round's fixes, never by the author's own pass; three rounds in, that pattern has not broken once.
