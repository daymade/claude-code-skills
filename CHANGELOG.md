...
- **transcript-fixer** (`daymade-audio` v1.27.0): hardens the full Native-first correction loop around four production contracts. (1) Context trap scan now executes both canonical `误识 → 正确` and legacy `误识 ≈ 正确` mappings, treats a fully quoted/backticked FROM phrase such as `CC 思维链` as one exact literal regardless of the bare-word length cap, and removes single-line `asr_note` values from its scan projection without introducing matcher-visible filler; title, keywords and body remain in scope. (2) Speaker-timestamp attribution lines are immutable across dictionary, context-rule, synchronous API and asynchronous API paths; explicit/generic labels, repeated bare labels, Latin name shapes, and CJK labels carrying Jieba's person-name tag are protected, while timestamp-ended prose remains editable. Damaged/moved markers fail closed, and internal markers are revealed before changes reach reports, history, or learning. Five real transcripts contributed 1,921 timestamp-shaped lines: all 1,920 attribution lines were protected and the one timestamp-ended metadata sentence was correctly left editable. (3) API fallback preserves exact inter-chunk separators, rejects empty/whitespace response text, retains failed chunks, and reports async failures as failures while still returning safely degraded text. A synchronous fallback-model success and asynchronous insert/delete corrections now enter the same precise change/history path as primary-model replacements; replayable word pairs can reach learning, while empty-sided or formatting-only records stay historical and never become dictionary rules. The extractor uses RapidFuzz's optimized Levenshtein opcodes: the old repetitive-token path took 6.9 seconds at 10,000 characters and exceeded 10 seconds at 50,000, while the replacement preserves the exact insert opcode without quadratic `SequenceMatcher` behavior. (4) Stage 1 auto-finalize removes only reproducible intermediates and retains `*_changes.md` / `*_needs_review.md` until every represented decision is closed. The Python 3.10 floor is also real across the dashboard metadata and concurrency manager: a loop timer covers semaphore acquisition plus the `async with` body without swallowing external cancellation. The runtime SKILL is now a compact decision layer over directly linked one-level references; stale re-anchor/API/dictionary/review pointers were repaired. Paired evaluation passes all three scenarios on the new version versus two on the immutable old version; only the trap-scan scenario is discriminating, while caller/re-anchor scenarios are preservation checks. Full suites pass on both Python 3.10 and 3.14 (`512 passed, 3 subtests` each); skill regression audit classifies and verifies all 760 candidates; public security and offline-link gates pass.
