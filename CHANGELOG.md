...
### Fixed
- **prior-work-retrieval** (daymade-claude-code v3.7.9 → v3.7.11): remove `uv run` from the synchronous Claude/Codex hook entrypoint. The wrapper now executes a concrete Python interpreter directly, so shared UV cache cleanup or lock contention cannot stall every PreToolUse decision; v3.7.10 remains reserved by an in-flight read-history change.
