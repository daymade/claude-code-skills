...
- **continue-codex-work** (`daymade-claude-code` v1.56.0): recover exact inherited fork lineage instead of treating a tiny child rollout whose only local prompt is `继续` as the task. The extractor follows every declared `history_base` edge root-first, reads each ancestor only through its recorded half-open byte snapshot, excludes parent events written after the fork, separates inherited actionable context from child-local turns, and auto-expands inherited summaries when the child contains only a continuation cue. Missing parents, cycles, mismatched IDs, out-of-range offsets, and offsets through a JSONL record fail visibly; a parent ID without a byte boundary is reported rather than guessed from the current full parent. Thirty-two regressions cover single/multi-level lineage, post-fork growth, archives, compaction, attachment boundaries, continuation cues, and malformed provenance; the Skill security scan and quick validation are green.
