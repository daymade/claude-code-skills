...
### Fixed
- **peer-message** v1.0.0 → v1.0.1: make documentation ownership executable instead of duplicative. `SKILL.md` now owns the stable Python runtime prerequisite and authorization boundary; `peer.py --help` owns CLI syntax; the protocol reference owns transport and verification; the official-feature reference owns volatile product requirements. README/README.zh-CN and `CLAUDE.md` point to those owners, while the changelog and private review stop persisting derived test, session, reachability, and file totals.
