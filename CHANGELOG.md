...
### Added
- **claude-code-history-files-finder** (`daymade-claude-code` v1.54.0): add an exact Codex session-ID metadata path and a bounded broad-scan contract. `analyze_sessions.py locate-codex <uuid>` now globs only filenames carrying that UUID, validates every candidate against authoritative `session_meta.id`, and reports active/archive copies without parsing unrelated rollout bodies; a sole UUID passed to `search ... --codex` auto-routes before any Claude or Codex corpus scan. Broad Codex keyword scans now emit 15-second progress heartbeats and stop after 300 seconds by default, exiting non-zero and refusing to label partial matches complete; `--codex-max-scan-seconds 0` is the explicit unbounded opt-in. Five new regressions cover exact-copy discovery, metadata/filename disagreement, pre-scan auto-routing, stop-loss, and heartbeat behavior; all 98 finder tests pass. On the motivating real store, exact lookup completed in 0.27 seconds instead of entering a multi-gigabyte full-history scan.
