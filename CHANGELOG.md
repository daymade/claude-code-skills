...
### Added
- **codex-1m-context-window-setup** (`daymade-codex` v1.1.0; marketplace v2.1.0): add a model-aware `doctor` / transactional `apply` / drift-detecting `verify` workflow for Codex CLI and Desktop's shared base config. It resolves the selected model and live `debug models` contract instead of guessing, requests up to 1,000,000 raw tokens while respecting the declared maximum, reports the effective usable window, and sets automatic compaction to 60% of the attainable raw window. The conservative TOML editor changes only `model_context_window` and `model_auto_compact_token_limit`, preserves comments and unrelated tables, creates content-addressed backups only for changed bytes, uses atomic same-directory replacement with a concurrency check, and restores the exact prior file when strict-config validation fails. Fourteen standard-library regressions cover model caps, the 1M/600K policy, idempotency, permissions, preservation, unsupported syntax, missing model contracts, and rollback for existing or newly created configs; the same suite passes on macOS and Linux.
