...
### Fixed
- **prior-work-retrieval** (`daymade-claude-code` v2.0.1): stop the prior-work gate from blocking its own receipt-completion command when a `--reuse` or `--reject` reason contains write-like prose such as `cp→symlink`. The exemption is now anchored to the parsed command entry instead of a route name found anywhere in the segment: quoted route names cannot launder arbitrary Python writes, unknown executors before a later retrieval command stay gated, and prefixed, chained, backgrounded, substituted, or direct tool writes remain fail-closed. Five new adversarial cases join the original regression; all 34 focused tests and the hook self-test pass.
