...
### Changed
- **continue-codex-work** (`daymade-claude-code` v1.50.0): fix silent briefing gaps against Codex 0.147 rollouts and give every truncation an exit. Measured on real rollouts: 0.147.0 removed the `event_msg/user_message` / `agent_message` mirror stream entirely (6/6 sampled 0.147.0 rollouts have zero such records), so the extractor's `Last User Requests` section silently vanished and assistant turns survived only via `task_complete` tails; the parser now reads turns from `response_item/message` (user `input_text` / assistant `output_text` decoded locally — the shared `extract_text` is deliberately untouched), prefers that stream whenever it is present so dual-stream 0.142.x rollouts never double-count, keeps the event stream as fallback, and never mistakes `response_item/agent_message` inter-agent encrypted traffic for main-thread text. Long sections (summary / requests / responses) now truncate with a printed `rerun with --full` hint instead of a silent "...", and `--full` prints complete text — the old Limitations line said "read the rollout line directly" without saying how, which cost a real resume session two wrong schema guesses. Skill-invocation injections (`<skill>` bundles delivered as user messages, measured ~90 KB) collapse to a one-line marker so they stop evicting real requests from the briefing window. 8 new synthetic-fixture tests pin the three schema shapes, inter-agent exclusion, the `task_complete` tail safeguard, the marker, and the truncation contract; real-corpus verified on a 187 MB / 22-compaction 0.147.0 rollout and a dual-stream 0.142.4 rollout.
