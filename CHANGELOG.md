...
### Added
- **claude-code-hooks** (`daymade-claude-code` v1.49.0): turns rule 7's hookless-loop lesson into an executable **Loop Contract**. Any Stop-hook remediation or agent-driven review/wait/retry loop must predeclare an immutable target+failure-axis key, fire condition T, remediation R, decreasing variant V, cycle budget, and distinct success/capped exits before cycle 1. Agent-driven independent review now defaults to one initial review plus one narrowly scoped re-review; unrelated findings become a new task instead of resetting the budget, and a same-axis blocker after the cap leaves the artifact explicitly blocked/unshipped instead of silently spawning a third reviewer. The prior three-round/~253K-token incident moves to pitfall #36, while the trigger-time Skill keeps only the contract and honest enforcement boundary. Verification: one bounded with-skill replay, fresh-context review, existing-skill regression audit, quick validation, and reference-net checks.
