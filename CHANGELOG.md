...
### Changed
- **transcript-fixer** v1.33.1 → v1.33.2: two exits that recorded something other than what happened. `harvest_corrections.py` aborted the whole harvest (exit 2, nothing printed) when one candidate could not be rendered as a trap bullet — a vendor's `***` redaction mask diffed against the real word yields a FROM the bullet grammar cannot carry, since `*` is the bold delimiter and `_BOLD_TRAP` rejects it on either side even inside backticks. On a real two-hour transcript that single pair hid every other candidate; the same pair now prints 45. The noise filter drops any side without lexical content or containing `*` (a mask can stand for any word, so `*** → 公众号` was never a trap), and a bullet that still fails the parser round-trip is reported on stderr, excluded from the output and from `--write`, and no longer aborts the run (`--json` lists it under `unrenderable`). The review queue's accept path fails closed with `ReAnchorNeeded` once the anchor text is gone; a fix applied by hand before the verdict was indistinguishable from file drift, and SKILL.md told the agent to close such rows with `kept_original` — a verdict that describes a transcript that was right as spoken, contradicting `native_ai_full_workflow.md` 9b, which reserves `kept_original` for the actual human verdict. `--decision accepted` now recognises the one shape it can tell apart: the context recorded at enqueue reappears within the ±3-line window with the suggestion in the exact slot the original occupied and its recorded neighbours intact (neighbourhood tried from 8 characters down to 2, so an adjacent correction on the same line does not defeat it; a one-sided neighbourhood needs at least 3). It records `accepted` without writing and logs `already in place at the anchor — recorded without writing`; any other shape still raises `ReAnchorNeeded`, the `asr_note` ledger mask still applies, and `reopen` reverts nothing it never wrote. SKILL.md, the native workflow reference and `script_parameters.md` now say to close accepted Stage 1 deferrals through their queue rows rather than with sed, and that `kept_original`/`skipped` are verdicts about the transcript, never a way past a stranded anchor. Nine deterministic tests cover both changes, including the ledger fixture whose previous assertion encoded the fail-closed exit; three pre-existing `test_stage1_auto_finalize` failures and three collection errors under `scripts/tests` are unchanged on the baseline and out of scope.
