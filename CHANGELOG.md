...
### Added
- **skill-governance** (`daymade-skill` v1.30.1): add a read-only project dual-root audit for `.claude/skills` and `.agents/skills`. It pairs direct child bundles by frontmatter `name`, hashes complete bundle content plus executable bits, reports identical copies without pretending they cannot drift later, and fails on divergent full copies. A thin compatibility router passes only when its exact marker is the first nonblank body line, it contains exactly one backtick-quoted paired canonical `SKILL.md` target (including directory names with spaces or non-ASCII text), a complete-read instruction, fail-visible behavior, and no second business-rules file. Shared targets pass only when the shared file is canonical and neither side hides extra material. Invalid identity/router state, broken nested symlinks, and declared-but-empty roots exit 2; real drift exits 1. All 21 standard-library regressions run in the shared CI registry and on Linux.
