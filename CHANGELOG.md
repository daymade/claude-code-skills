...
- **mindmap-markmap-viewer** v1.1.0: new skill — turn Markdown outlines/notes into an interactive mind map as ONE self-contained offline HTML (markmap.js vendored + inlined, white-on-dark), with a zoom/fit/expand/collapse toolbar, SVG/PNG export (2× raster, snapshots fold state), expand-by-level control, and accent-insensitive context-preserving search (match + ancestors + descendants). Zero runtime deps (Python stdlib); hardened against a 12-finding adversarial review; 41-check dependency-free regression suite. [Live demo](https://jaderson-bit.github.io/mindmap-markmap-viewer/).
