...
### Changed
- **macos-cleaner** v1.3.0 → v1.4.0: adds a targeted Apple Content Caching / `AssetCacheManagerUtil` workflow grounded in a real macOS 26.5.2 repair and Apple's supported command-line controls. It distinguishes logical `CacheUsed` from physical `ActualCacheUsed`, verifies peers without touching them, protects co-resident services, separates read-only diagnosis → plan/confirmation → supported mutation → APFS/service verification, and fail-fasts on partial deactivation or purge. The main SKILL.md is now a compact route-and-safety layer instead of a duplicated manual; the Mole, cleanup-target, report, and safety references follow the same phase contract instead of retaining broad immediate-cleanup instructions. The unknown-source route restores an ordered short-circuit sequence, establishes an explicit success target, names every fixed scanner scope, and requires an exact approved path for content scans; optional duplicate detection is read-only and path-scoped. The original Xcode/npm/uv/Playwright/iOS/model/JetBrains cache trade-offs and narrow npm `_npx` route remain reachable from named-cache requests. Docker metadata remains read-only, while temporary-container volume inspection has a separate no-pull/no-network/read-only authorization gate and object-size reports permit honest unknown/upper-bound host release estimates. `cleanup_report.py` is wired back into local before/after verification, defaults to `/System/Volumes/Data`, records the measured volume and offset-aware timestamp, returns nonzero on baseline failure, and refuses legacy, stale-order, or cross-volume comparisons; nine deterministic tests cover that contract. The contradictory `docker builder prune` exception is gone: build cache remains measurable but its prune-based deletion is explicitly outside the skill's execution scope.
