...
- **continue-codex-work** (`daymade-claude-code` v1.56.0): recover exact inherited fork lineage instead of treating a tiny child rollout whose only local prompt is `继续` as the task. The extractor follows every declared `history_base` edge root-first, reads each ancestor only through its recorded half-open byte snapshot, excludes parent events written after the fork, separates inherited actionable context from child-local turns, auto-expands every ancestor's retained summary when the child contains only a continuation cue, and prints progress before parsing each potentially large inherited prefix. Inherited turns now render as a record-ordinal chronological handoff timeline: every retained user correction stays attached to the first/latest assistant state before the next request, so an old final answer cannot be displayed beside a later request as if it answered it; `--full` removes the default 40-user-segment timeline cap. Missing parents, cycles, mismatched IDs, out-of-range offsets, offsets through a JSONL record, and a selected rollout whose `session_meta.id` differs from the requested session fail visibly; a parent ID without a byte boundary is reported rather than guessed from the current full parent. All 36 tests pass, including post-fork growth exclusion, continuation-only summary expansion, chronological correction/state pairing, final-answer placement, full timeline expansion, split offsets, conflicting parent IDs, selected-rollout identity, cycles, and missing byte boundaries. A real two-ancestor replay recovered the previously hidden system-level objective in default mode while excluding a post-fork parent tail. Skill Creator's immutable-baseline audit classified 8 changed candidates with 239 exact preservations and passed; public quick validation and the security scan are green.
