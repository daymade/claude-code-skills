...
⚠️ **On `source.html`, "empty" is not a literal stop condition.** `source.html` is an immutable raw capture — nothing in this skill rewrites it in place — so a hub doc that genuinely references N children or M diagrams keeps showing N+M matches forever, even after every one is correctly handled; chasing this grep to a literal zero on `source.html` will never terminate for a real hub. Treat each match as a worklist item: a `mention-doc`/`cite doc-id=`/`sheet` hit is resolved once the referenced child has actually been fetched and saved; a `whiteboard` hit is resolved once its preview `.jpg` was exported and Read (dispatch-table row above) — it is not a missing document to fetch, don't treat it as one. This grep still recurses over every file already on disk, so it naturally covers the raw `.html` saved in Step 3 alongside the pandoc-converted `.md` — no extra step needed there. (On the `.data.markdown` branch, where the fetched body is the deliverable Markdown itself, a literal empty result remains the simpler signal.)
