...
Each line is one JSON object with `at` (ISO 8601 with timezone), `lane_id`, `state`, and `note` (brief observation). State transitions are `prepared → submitted → running → collected`; `submitted → collected` is valid when the provider offers no running state. `prepared`, `submitted`, or `running` may lead to `deferred` or `failed_unknown`; either can return to `submitted` for a verified retry. The origin cannot change between submission, running and collection within one run; a retry after `deferred` or `failed_unknown` may start a new origin. Never turn a timeout into a second paid request merely to fill the ledger: query an existing provider task ID first. A later export for a collected lane is another `collected` event with a distinct file, preserving the original.
