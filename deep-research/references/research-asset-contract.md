...
1. Choose the owning project, durable study path and explicit project catalog. Run `search` on entity names, aliases and the business question. By default it searches approved originals and labeled legacy sources; use `--include-leads` only when the verified index is insufficient and inspect the candidate status before reuse. Open any relevant prior original or claim before deciding reuse; search hits are locators. If this is the first study, an empty catalog is a valid recorded result.
