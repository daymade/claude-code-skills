...
description: >-
  ...
  autonomy threshold, the requirement-vs-method split line, the five resolved
  scope boundaries that look like contradictions but are not, and the four
  agent-orchestration questions with no default count. Read at Step 4
...
## Five Resolved Scope Boundaries
...
Agent count is an output of these four questions, not a preset default. No
question has a standing answer, and no fan-out number is the default — a
preset cluster size is an anti-pattern:

1. **How long will it take?** < 10 min → do it yourself. > 30 min → spawn.
   10–30 min → weigh the remaining three questions.
2. **Does it need main-conversation context** (user preferences, multi-round
   feedback, nuanced decisions)? Yes → do it yourself.
3. **Does it need an unbiased third party** (evaluator, reviewer)? Yes → must
   spawn, even when fast.
4. **Is it truly parallel** (independent streams)? Yes → must spawn. Otherwise
