...
**Why this exists instead of `codex resume`**: replaying a full rollout re-feeds every reasoning step, tool call, and tool output back into the context window. For long sessions that wastes the window on resolved turns and stale output. This skill **selectively reconstructs** only actionable context — inherited fork snapshots, raw pre-compaction records still present on disk, the latest compaction's surviving context, recent user/assistant turns, the tool calls and files edited, and how the session ended — giving a fresh start with prior knowledge.
...
- `## Inherited Actionable Context` — each ancestor's last retained compacted context plus a record-ordinal **chronological handoff timeline**, latest plan, tool calls, and edited files, kept separate from the selected child's local turns. The timeline includes every retained user turn and the first/latest assistant state before the next user turn, so an old final answer cannot be displayed beside a later request as if it answered it. If the child's only prompt is a continuation cue, inherited compacted context auto-expands even without `--full`; making the agent manually discover that escape hatch would recreate the original failure.
- `## Compacted Context` — if the session was compacted, the full retained user/assistant thread from the last `message` / `replacement_history`; system preamble and re-injected `AGENTS.md` are stripped out. Default output clips this section with a visible `--full` escape; parsing itself does not pre-truncate it.
...
5. Read the `Recovery boundary` literally: the extractor is compaction-aware and reads both raw pre-compaction records still on disk and surviving `replacement_history`. It cannot reconstruct content absent from both sources, restore omitted attachment bytes, or prove old tool claims. Verify the current workspace.
...
- **Compaction** from `compacted` records — Codex replaces the live model window with a `replacement_history` of messages (not a single summary), and re-injects the system preamble. The parser ingests every compacted record, keeps its full retained user/assistant text, filters harness noise, and also continues reading any raw pre-compaction records still present earlier in the rollout. The briefing renders only the latest compacted context for the selected session and for each ancestor, because that is the continuation state that superseded earlier compacted windows. Default briefing clipping happens only at render time, so `--full` can expose all text retained in that latest state.
...
- Do not treat compacted context or tool output as complete truth — compaction preserves continuation state, not a verbatim guarantee. Always verify claims against the current workspace.
...
- Compaction is supported, not reversed: raw pre-compaction records still present in the rollout remain eligible for the chronological timeline; every `compacted` record is ingested, while the briefing renders the latest retained `message` / `replacement_history` per selected session or ancestor. A detail absent from both on-disk sources cannot be reconstructed. `--full` removes extractor-side character clipping from that retained latest context; image-only turns remain markers and do not recreate the original image/audio bytes.
