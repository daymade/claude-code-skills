...
- **Compaction** from `compacted` records — Codex replaces the live model window with a `replacement_history` of messages (not a single summary), and re-injects the system preamble. The parser keeps the full retained user/assistant text, filters harness noise, and also continues reading any raw pre-compaction records still present earlier in the rollout. Default briefing clipping happens only at render time, so `--full` can expose all retained text.
...
- Compaction is supported, not reversed: raw pre-compaction records still present in the rollout remain eligible for the chronological timeline, and each `compacted` record's retained `message` / `replacement_history` is rendered separately. A detail absent from both on-disk sources cannot be reconstructed. `--full` removes extractor-side character clipping from retained compacted text; image-only turns remain markers and do not recreate the original image/audio bytes.
