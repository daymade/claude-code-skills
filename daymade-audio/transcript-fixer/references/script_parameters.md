...
- `--json` (optional): Emit the Stage 1 contract fields (`applied`, `deferred`, `output_path`, `needs_review_path`, `input_unchanged`, `review_enqueued`) plus additive `stage1_only_incomplete`. A Stage 1 run always reports that field as true; the caller must run Native AI or explicitly choose the agent-less Stage 2/3 route before claiming end-to-end completion. Stage 2/3 runs add `stage2_total_chunks`, `stage2_failed_chunks`, and `stage2_degraded`; any failed chunk is retained from source and makes `stage2_degraded` true, so automation must not equate exit 0 with a fully corrected API pass.
