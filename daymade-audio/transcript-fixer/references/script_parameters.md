...
- `--resolve-review ID --decision accepted|overridden|kept_original|skipped|reopen`: Record a verdict and execute the action pack (`overridden` requires `--override-to TEXT`; `--note` free-text evidence; `--by` reviewer name; `reopen` reverts applied edits and re-pends the item). When the suggestion is already in place at the anchor — a hand edit landed before the verdict — `accepted` records the verdict without writing and logs `already in place at the anchor — recorded without writing`
