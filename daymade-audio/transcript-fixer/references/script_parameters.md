...
- `--resolve-review ID --decision accepted|overridden|kept_original|skipped|reopen`: Record a verdict and execute the action pack (`overridden` requires `--override-to TEXT`; `--note` free-text evidence; `--by` reviewer name; `reopen` reverts applied edits and re-pends the item). When the resolved text is already in place — a hand edit landed before the verdict, the original is gone from the whole file, and the recorded context reappears with the text in its slot — `accepted` and `overridden` record the verdict without writing and log `already in place at the anchor — recorded without writing`; a third form in that slot elsewhere, or an edit touching the slot's neighbours, still exits 2 with `re_anchor_needed`
