...
Stage 1 also refuses a dictionary match on its own at three points before risk scoring: the superset check (the corrected form is already in place), the common-word boundary check for short rules, and the word-boundary check — a dictionary-only jieba segmentation of the neighbourhood, where a match that begins or ends inside a multi-character segment (新一 in 更新|一下, 同龄 in 同龄人, `Cloud` in `iCloud`) is a fragment of real words, not a mishearing, and is counted as `Refused at word boundaries` instead of applied or deferred. Its blind spot is a real mishearing whose neighbours happen to complete two ordinary words; safe mode only ever deferred that class, and the native read-through still owns it. The layers are listed in [references/false_positive_guide.md](references/false_positive_guide.md).
...
- Preserve raw transcripts, `*_changes.md`, and `*_needs_review.md` as evidence until `--close-sidecars --input "<absolute-canonical-file>"` reports `closed`: every entry reads applied in the file (or its anchor was rewritten past both forms) or carries a non-pending queue verdict for this exact file, and the file has zero pending rows. It exits 1 (`open`) naming the undecided entries and pending ids, 2 (`blocked`) when a `*_stage1.md` newer than the file still awaits the plain Stage 1 rerun, and 0 after removing the evidence and stale run outputs. For an entry that still reads as the original and has no row at all, `--decide-raw kept_original|skipped --by <who> --note <why>` records the verdict through the queue at closure — an audit trail, not a silent deletion. `--dry-run` shows the verdict without deleting; `--json` returns it.
