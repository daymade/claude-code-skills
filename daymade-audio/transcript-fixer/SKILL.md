...
4. **Run Stage 1 and inspect the real result.** Prefer explicit project domains plus `--apply-domain --json`. Read `deferred` and `review_enqueued`; never silently discard the sidecar or queue gap. Close a deferral you agree with through its queue row (`--resolve-review <id> --decision accepted`): the accept path applies the edit itself, or — when a hand edit already landed the suggestion at the anchor — records the verdict without writing. Either way the row ends `accepted`; `kept_original` describes a transcript that was right as spoken, and is not a way to close a row whose fix you applied.
...
- A single-line `asr_note` ledger is masked on the accept path too, so resolving an item never edits the provenance line that cites the old form. If you applied the fix by hand before resolving, `--decision accepted` still records the verdict without writing when the recorded context now reads with the suggestion in place (`already in place at the anchor — recorded without writing`); when the anchor line no longer matches even with the suggestion substituted, the item fails closed with `ReAnchorNeeded` — run `--reanchor-review <id>` if the file drifted. `kept_original` and `skipped` remain verdicts about the transcript, never a way past a stranded anchor.
