...
description: >-
  Corrects speech-to-text transcription errors using dictionary rules and Claude's built-in AI (no external API key required — Native AI Correction is the DEFAULT). Stage 1 alone is not the job. Stage 3 API is a backup for automation without Claude Code. Builds personalized correction databases that learn from each fix, auto-loads person-name ASR variants from the global people roster (config.json people_roster_path → PKM next/_meta/people.md), but never auto-loads project-level name rosters — before any person-name correction Claude must itself consult BOTH rosters: the global PKM people.md (its ASR 变体 lines feed Stage 1, yet DB-disabled/suppressed entries and unlisted people still require a direct read) and the owning project's roster (e.g. huawei-green-energy/context.md people tables), and must never settle a name by majority occurrence in the text, and reads per-domain context files that prime the AI pass for context-dependent homophones. Triggers when working with ASR/STT output containing recognition errors, homophones, garbled technical terms, person-name errors, or Chinese/English mixed content. Also triggers on requests to clean up meeting notes, lecture transcripts, interview recordings, or any text produced by speech recognition. Use this skill even when the user just says "fix this transcript", "clean up these meeting notes", or mentions garbled names without invoking ASR specifically.
---
